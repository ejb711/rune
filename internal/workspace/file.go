...
func (f *file) flush(force bool) error {
	...
	if !f.infoModTime.Equal(savedModTime) {
		reopenedModTime := f.infoModTime
		// A rename may change mtime, but an intervening external write
		// must not become our watcher or stale-save baseline.
